...
import com.phi.loyalty.cashback.domain.campaign.api.v1.web.request.CashbackRequest;
import com.phi.loyalty.cashback.domain.campaign.api.v1.web.request.CreateCampaignRequest;
import com.phi.loyalty.cashback.domain.campaign.api.v1.web.request.ModifyCampaignRequest;
import com.phi.loyalty.cashback.domain.campaign.api.v1.web.request.SponsorRequest;
import com.phi.loyalty.cashback.domain.campaign.api.v1.web.request.UpdateDueDateRequest;
...
import com.phi.loyalty.cashback.domain.campaign.service.CampaignService;
...
import com.phi.loyalty.cashback.infra.common.constants.WebConstant;
import com.phi.loyalty.cashback.infra.common.enums.ErrorCodeEnum;
import com.phi.loyalty.cashback.infra.common.handler.GlobalExceptionHandler;
import com.phi.loyalty.cashback.infra.common.util.JsonUtil;
...
class CampaignControllerTest {
    ...
    private CashbackRequest cashbackRequest;
    private SponsorRequest sponsorRequest;
    private Campaign campaign;
    private Cashback cashback;
    private Platform platform;
    private Sponsor sponsor;
    private UpdateDueDateRequest updateDueDateRequest;
    private ModifyCampaignRequest modifyCampaignRequest;
    private String authorizationHeader;
    private String userInfo;

    @BeforeEach
    void initEach() {
        mockMvc = MockMvcBuilders.standaloneSetup(campaignController).setControllerAdvice(new GlobalExceptionHandler()).build();
        sponsorRequest = SponsorRequest.builder().document("93833615060").build();
        sponsorRequest = SponsorRequest.builder().document("93833615060").build();
        cashbackRequest = CashbackRequest.builder().maximumPaymentValue(500L)
                .percentage(new BigDecimal("0.1")).build();
        createCampaignRequest = CreateCampaignRequest.builder().name("Test Cashback").startsAt(LocalDate.now().plusDays(1))
                .endsAt(LocalDate.now().plusMonths(1)).cashback(cashbackRequest).sponsor(sponsorRequest).paymentType(PaymentTypeEnum.PAYMENT_SLIP).build();
        platform = Platform.builder().id(1L).name("FortPay").balanceType("FortPay").createdAt(LocalDateTime.now()).build();
        sponsor = Sponsor.builder().id(332040L).document("93833615060").platform(platform).createdAt(LocalDateTime.now()).build();
        cashback = Cashback.builder().id(1L).maximumPaymentValue(500L).percentage(new BigDecimal("0.1"))
                .createdAt(LocalDateTime.now()).build();
        campaign = Campaign.builder().id(1L).name("Test Cashback").startsAt(LocalDate.now()).endsAt(LocalDate.now().plusMonths(1))
                .sponsor(sponsor).cashback(cashback).status(CampaignStatusEnum.CREATED).createdAt(LocalDateTime.now()).paymentType(PaymentTypeEnum.PAYMENT_SLIP).build();
        updateDueDateRequest = UpdateDueDateRequest.builder().endsAt(LocalDate.now().plusDays(1)).build();
        modifyCampaignRequest = ModifyCampaignRequest.builder().name("teste").startsAt(LocalDate.now().plusDays(1)).endsAt(LocalDate.now().plusDays(1)).cashback(cashbackRequest).build();
        authorizationHeader = "Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJDeDRQVUtFQlFZRmEyWU5FWXhjWG82V3c5Vmo2cXhwdm1QdHpGQVQ4YkM4In0..bQAlQ8dgaM_oiLM9Pogr3f1nLzDn4x4Fi4Ygx4S3Fa_D-xda4EWVo_vIKh8W9EMS7rKrYwqnlF6HBuCMNLpQF2zq2CBwenFxJ52Ne1y-JjzNqtWEDH0mthwJYGIyF-v7-AJT-yV48ghl6lausWYPWJZZDZpVzDAu94wVyxk0Tcc4pUTeE03IjqfR1Njl4fE-JKT8Jf7LvXZM89xGVnmtYPqa452WMMRq8wQlVWVQP3irEIrDgmoNdXawTI21K8b7-1rAk7mXshEB75J6UvXLAMXthizxBon4k25mfUhGs-q-OneXQ5WhYOADEVLhBnN_B5iwOcrAEXsHkxbWo8ER4w";
        userInfo = "Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJwM3MzaDZnOXdjMGxSbTBTYlZIMmg1ajEwclB2TXVSWVg0dFhfYlgxblVnIn0..H5jSIPNMjHCstb8eU1Y4R8tS7wFp-UAsX2C8xUnQ00ua2M_DonBYbG-YN--jjxIWfa1Mki3e5YnMJ1F8hr9FlFQvIZk0n-kla1ysFE0YO3Z_zr19b8G-ZjvGzW5hLD5HMGcC-g8oFRxaWsArvoogy_vE0agJYxu1ifVDXHyuJBBWGk5L_UPKXgU6rWuC3jM0DrgBVezLC55elBTXJoODQX7s_t5qc0Lq19epy_DVzj1liIMINFVrtssxLzn15IRD6fwAUtx3nLHxQVWkw88u4f4cAMG6PN5mzv2Q9l2iTDO-YfrZkROIo8DdK_6p6z4Kl4vpDPspJt6Btxn3LUWpzQ";
    }
